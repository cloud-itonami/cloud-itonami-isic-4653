(ns agmachtrade.phase
  "Phase 0->3 staged rollout for the agricultural-machinery-wholesale
  actor.

    Phase 0  read-only        -- no writes, still governor-gated.
    Phase 1  assisted-intake  -- equipment-order intake allowed, every
  ...
  real physical farm machinery from the wholesale yard/dealership to a
  counterparty and settling a real ag-machinery invoice (real money
  moving between counterparty and wholesaler) are the two real-world
  physical/financial acts this actor performs; both are always a human
  trading supervisor's call. `agmachtrade.governor`'s `:delivery/
  dispatch`/`:invoice/settle` high-stakes gate enforces the same
  ...
  distinct from the equipment-order itself.")
...
(defn verdict->disposition
  "Map an Ag Equipment Governor verdict to a base disposition before the
  phase gate."
  [verdict]
