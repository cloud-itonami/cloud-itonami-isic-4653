(ns agmachtrade.operation
  "OperationActor -- one agricultural-machinery-wholesale operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (AgMachTradeAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Ag Equipment Governor
  ...
  One graph run = one agricultural-machinery-wholesale operation
  (intake -> advise -> govern -> decide -> commit | hold | approval). No
  unbounded inner loop -- each operation is auditable and checkpointed.
  An equipment-order's life is advanced by MANY ops (intake / verify /
  ...
  decision to a human operator (trading supervisor / yard operator).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:delivery/dispatch`/`:invoice/settle` ALWAYS reach this
  node when the governor is clean -- see `agmachtrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [agmachtrade.agmachtradeadvisor :as agmachtradeadvisor]
...
(defn build
  ...
  `agmachtrade.store/Store`).
  opts:
    :advisor      -- an `agmachtrade.agmachtradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (agmachtradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AgMachTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (agmachtradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(agmachtradeadvisor/trace request p)]})))

      ;; Ag Equipment Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
